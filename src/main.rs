...
fn main() {
    ...
    generate_random_undirected_graph(8000 ,5,100,false,"test").expect("error");
    perform_hp_search("out/test",1).expect("error");
}

fn generate_random_undirected_graph(num_nodes:usize,  min_degree:usize,max_degree: usize,directed:bool, output_file:&str) -> std::io::Result<()> {
...
fn brute_force_hamilton_path(nodes_edges:&HashMap<usize,Vec<usize>>){
...
fn dyn_subset(graph:&Vec<Vec<bool>>)->bool{
    let n = graph.len();
...
fn init_bit_graph(graph:&mut Vec<Vec<bool>>, nodes_edges:&HashMap<usize,Vec<usize>>) {
...
fn brute_force_hp_rec(graph:&Vec<Vec<bool>>,mut permutation : Vec<bool>,node:usize)->bool{
